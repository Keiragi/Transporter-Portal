...
# 接続判定
  execute unless entity @s[tag=S_Portal.Connected] run data modify storage shrs: S_Portal_Maker.Info merge value {"state":"None","color":"gray"}
...
# 転移先が埋まっていないか確認
  ...
  data modify storage shrs: S_Portal_Maker.Info merge value {state:"Block",color:"dark_red"}

# 出力
  function s_portal:portal/touch/attack/message with storage shrs: S_Portal_Maker.Info
  title @a[limit=1,advancements={s_portal:touch={attack=true}}] actionbar {"text":"ℹ 右クリックで除去","color":"white"}

# 初期化
  data remove entity @s attack
  data remove storage shrs: S_Portal_Maker.Info
